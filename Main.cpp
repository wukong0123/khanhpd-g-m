...
int main(int argc, char *argv[]){
    ...
    SDL_Renderer* renderer = CommonFunc::createRenderer(window);

    SDL_Texture* BackGround = CommonFunc::loadTexture("Back_ground.jpg", renderer);
    ...
    while ( true ) {
        ...
        while ( List_enemy.size() < 4 ){
            Enemy* Su = new Enemy();
            Su->setTexture("Enemy.png" , renderer) ;
            Su->MOVETO( 500 , 325 , Su->getSP() ) ;
        ...
        for ( int i = 0 ; i < List_enemy.size() ; ++ i ){
            ...
            Enemy* C_enemy = L_list.at(i) ;
             if ( C_enemy->is_exist() ){
                ...
                //SDL_RenderPresent( renderer );
                C_enemy->move() ;
                //std :: cout << C_enemy->getX() <<' ' << C_enemy->getY() <<' ' << C_enemy->getDX() <<' ' << C_enemy->getDY() << "\n" ;
                if (  C_enemy->Coll(Mine) ){
                    C_enemy->updHP() ;
                    Mine.updHP() ;
                }
